/* =========================================================
   INTERRIORS — variation 2 "Swiss Grid Mono" script
   Adds: global scroll-progress bar, horizontal stepper
   progress fill (roadmap). Core UX kept from original.
   ========================================================= */
(function () {
  'use strict';
  ...
  /* Header scroll state + scrollspy + global scroll progress */
  ...
  function onScroll() {
    ...
    var offset = (header ? header.offsetHeight : 76) + 60;
    var current = null;
  ...
  /* --------------------------------------------------
     NEW: Horizontal stepper progress line (roadmap)
  -------------------------------------------------- */
  var stepperTrack = document.getElementById('stepper-track');
  var stepperFill  = document.getElementById('stepper-fill');
  function updateStepperFill() {
    if (!stepperTrack || !stepperFill) return;
    var max = stepperTrack.scrollWidth - stepperTrack.clientWidth;
    var pct = max > 0 ? stepperTrack.scrollLeft / max : 0;
    stepperFill.style.width = (pct * 100) + '%';
  }
  if (stepperTrack) {
    stepperTrack.addEventListener('scroll', updateStepperFill, { passive: true });
    updateStepperFill();
  }
